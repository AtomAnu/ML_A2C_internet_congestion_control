...
try:
    for i in range(iterationNum):
        ...
        info = None
        current_state = [obs[8]*0.00001,obs[9]*0.001,obs[11]*0.00001,obs[13]*(1/340)*0.001,obs[14]*(1/340)*0.001]
        print(current_state)
        score = 0
        throughput_sum = 0
        rtt_sum = 0
        ...
        while True:
            ...
            current_state_utility = 10*math.log1p(obs[15])-0.1*math.log1p(obs[11])

            obs, reward, done, info = env.step([new_ssThresh, new_cwnd])

            next_state_utility = 10 * math.log1p(obs[15]) - 0.1*math.log1p(obs[11])
            ...
            next_state = [obs[8]*0.00001,obs[9]*0.001,obs[11]*0.00001,obs[13]*(1/340)*0.001,obs[14]*(1/340)*0.001]
