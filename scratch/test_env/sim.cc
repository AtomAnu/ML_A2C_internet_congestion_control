...
//  bool flow_monitor = false;
  ...
  */
  ...
  if (transport_prot.compare ("ns3::TcpRl") == 0)
  {
    ...
    Config::SetDefault ("ns3::TcpRl::Reward", DoubleValue (4.0)); // Reward when increasing congestion window
...
//
//  ApplicationContainer clientApp = ftp.Install (d.GetLeft (0));
//  clientApp.Start (Seconds (start_time * 0)); // Start after sink
//  clientApp.Stop (Seconds (stop_time - 3)); // Stop before the sink
//
//  AddressValue remoteAddress (InetSocketAddress (d.GetRightIpv4Address (1), port));
//  Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (tcp_adu_size));
//  BulkSendHelper ftp ("ns3::TcpSocketFactory", Address ());
//  ftp.SetAttribute ("Remote", remoteAddress);
//  ftp.SetAttribute ("SendSize", UintegerValue (tcp_adu_size));
//  ftp.SetAttribute ("MaxBytes", UintegerValue (data_mbytes * 1000000));
//
//  ApplicationContainer clientApp = ftp.Install (d.GetLeft (1));
//  clientApp.Start (Seconds (start_time * 1)); // Start after sink
//  clientApp.Stop (Seconds (stop_time - 3)); // Stop before the sink
