...
TcpGymEnv::GetActionSpace()
{
  ...
  std::string dtype = TypeNameGet<float_t> ();
...
{
  Ptr<OpenGymBoxContainer<float_t> > box = DynamicCast<OpenGymBoxContainer<float_t> >(action);
...
TcpTimeStepGymEnv::GetObservation()
{
  ...
  Ptr<OpenGymBoxContainer<float_t> > box = CreateObject<OpenGymBoxContainer<float_t> >(shape);
  ...
  //throughput  kbytes/s
  float throughput = (segmentsAckedSum * m_tcb->m_segmentSize) / (m_timeStep.GetSeconds() * 1000);
