...
try:
    for i in range(iterationNum):
        ...
        info = None
        # current_state = [obs[8],obs[9],obs[11],obs[13],obs[14]]
        # current_state = [obs[8]*0.0001,obs[9]*0.001,obs[11]*0.00001,obs[13]*(1/340)*0.001,obs[14]*(1/340)*0.001]
        # current_state = [obs[7] * 0.001, obs[9] * 0.001, obs[13] * (1 / 340) * 0.0001, obs[14] * (1 / 340) * 0.0001]
        ...
        while True:
            ...
            # print('CWND increase: {}'.format(0.03*stepIdx*i))
            # print('ssThresh increase: {}'.format(action[s1]))
            ...
            # current_state_utility = math.log1p(obs[15])
            # print('Current Utility: {}'.format(current_state_utility))
            # print('Current TP: {}'.format(math.log1p(obs[15])))
            # print('Current RTT: {}'.format(0.01*math.log1p(obs[11])))
            ...
            # print('Next State Utility: {}'.format(next_state_utility))
            # print('Next TP: {}'.format(math.log1p(obs[15])))
            # print('Next RTT: {}'.format(0.01 * math.log1p(obs[11])))
            # next_state_utility = math.log1p(obs[15])
            ...
            # next_state = [obs[8]*0.0001,obs[9]*0.001,obs[11]*0.00001,obs[13]*(1/340)*0.001,obs[14]*(1/340)*0.001]
            # next_state = [obs[7] * 0.001, obs[9] * 0.001, obs[13] * (1 / 340) * 0.0001, obs[14] * (1 / 340) * 0.0001]
            # next_state = [obs[8],obs[9],obs[11],obs[13],obs[14]]
            ...
            if stepIdx < n_step:
                # print('LESS: {}'.format(str(stepIdx)))
                accum_reward += (a2c_agent.gamma ** (stepIdx-1)) * reward
                # print("STATE LIST LEN: {}".format(len(state_list)))
            elif stepIdx == n_step:
                # print('TEN: {}'.format(str(stepIdx)))
                accum_reward += (a2c_agent.gamma ** (n_step-1)) * reward
                a2c_agent.learn(state_list[0], accum_reward, next_state, done)
                state_list.pop(0)
                # print("STATE LIST LEN: {}".format(len(state_list)))
            else:
                # print('MORE: {}'.format(str(stepIdx)))
    ...
    plt.xlabel('Action')
    plt.savefig('action0_occurrence_'+str(n_step)+'_step.png')
    ...
    plt.savefig('CWND_trend_'+str(n_step)+'_step.png')
